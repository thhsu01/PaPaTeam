...
收斂已完成，19 頁的結構現在都符合本規格；仍未補齊的內容缺口見文末「尚未收斂的一件事」。
...
**19 頁三個介面現已全部收斂，並由 `tools/spec_sweep.py` 逐區塊檢查**：`chart`、`marker`、
...
19 頁已全數符合本規格的結構部分（2026-08-02 起新增的 `hushan`、`nangangshan`、
`datongshan`、`shanying`，以及 2026-08-03/04 補建的 `jiantanshan`、`jinmianshan`、
`caolingguidao`、`henglingguidao`、`zhongzhengshan` 是照本規格從零建起、而非事後收斂的）。以瀏覽器
...
| 項目 | 檢查方式 | 狀態 |
|---|:--:|---|
| 段落 id 與順序 | 🤖 | ✅ 19/19 為 `overview` → `map-section` → `elevation` → `spots` → `timeline` |
| 導覽鍵（五顆、規格文字） | 🤖 | ✅ 19/19（2026-08-04 收斂：先前 7 頁用規格外的「實走紀錄」、5 頁用「實際行程」，已統一為「實走紀錄」並同步改規格表）|
| `<h2>` 主詞符合規格表 | 🤖 | ✅ 19/19（2026-08-04 收斂：`map-section` 5 頁、`elevation` 5 頁、`timeline` 2 頁曾各自命名）|
| 航點卡欄位 id | 🤖 | ✅ 19/19（含 `wp-pos-label`、`wp-time`、`wp-advice`）|
| canvas id 為 `elevation-chart` | 🤖 | ✅ 19/19 |
| 灰階用 stone | 🤖 | ✅ 19/19 — **class 名稱與十六進位值都查過**，且先剝掉註解再查 |
| 總覽有總里程 | 🤖 | ✅ 19/19（2026-08-04：`bishan` 標「徒步長度」不符規格，已改「總里程 km（估）」）|
| 站徽帶 `aria-label` | 🤖 | ✅ 19/19 |
| 無左上 `fixed` 返回鍵 | 🤖 | ✅ 19/19 |
| 每個航點都有 `advice` | 🤖 | ✅ 19/19，共 188 個航點 |
| 里程健全性（≥ 直線、單調） | 🤖 | ✅ 19/19，0 筆 |
| 最高點語意色為 `#7c9e52` | 🤖 | ✅ 19/19（2026-08-04 收斂：5 頁曾各自用琥珀／深綠／橘／紅，且都在註解裡自稱「語意色」）|
| 導覽列 `fixed` | 👁 | ✅ 19/19 |
| `<footer>` 在 `<main>` 外 | 👁 | ✅ 19/19 |
| 地圖單欄堆疊 | 👁 | ✅ 19/19 |
| 無 `scrollToSection()` | 👁 | ✅ 19/19 |
| 站徽（綠圓 + 爬爬小隊） | 👁 | ✅ 19/19 |
| 段落 `scroll-margin-top` | 👁 | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | 👁 | ✅ 19/19 導航鍵與段落色條（2026-08-04 收斂：7 頁的導航鍵、6 頁的段落色條曾寫死 `bg-blue-600`／`bg-indigo-600` 等）。深色專題面板與語意色仍在例外之列，見「尚未收斂的一件事」|
| 小字對比 ≥ 4.5:1 | 👁 | ✅ 19/19（含 index）0 筆失敗。已知量測假陽性：skip link（聚焦前在畫面外）、index 的「詳情待補充」徽章、以及字佔滿整格的大標題（量到的底色會等於字色本身）|
...
目前**沒有內容缺口**。`advice` 全部 19 頁、每個航點都有（定義見下）。
...
**最後更新**：2026-08-04（新增 `zhongzhengshan`，19 頁結構已全數收斂；導覽文字、`<h2>` 主詞與最高點語意色收斂，並加上 `tools/spec_sweep.py` 讓這張收斂表可被機器複驗）  
