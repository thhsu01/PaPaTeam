...
收斂已完成，16 頁的結構現在都符合本規格；仍未補齊的內容缺口見文末「尚未收斂的一件事」。
...
| 項目 | 狀態 |
|---|---|
| 段落 id 與順序 | ✅ 16/16 為  → `map-section` → `elevation` → `spots` → `timeline` |
| 導覽鍵（五顆、規格文字） | ✅ 16/16 |
| 航點卡欄位 id | ✅ 16/16（含 `wp-pos-label`、`wp-time`、`wp-advice`；`huoyianshan` 的建議框 2026-08-03 補上，此前是唯一缺的一頁）|
| canvas id 為 `elevation-chart` | ✅ 16/16 |
| 導覽列 `fixed` | ✅ 16/16 |
| `<footer>` 在 `<main>` 外 | ✅ 16/16 |
| 地圖單欄堆疊 | ✅ 16/16 |
| 灰階用 stone | ✅ 16/16 — **class 名稱與十六進位值都查過**（2026-08-03 起）|
| 總覽有總里程 | ✅ 16/16 |
| 無 `scrollToSection()` | ✅ 16/16 |
| 站徽（綠圓 + 爬爬小隊） | ✅ 16/16 |
| 段落 `scroll-margin-top` | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | ✅ 16/16 |
| 小字對比 ≥ 4.5:1 | ✅ 16/16（含 index）0 筆失敗，2026-08-03 第八輪全面複測 |
| 無左上 `fixed` 返回鍵 | ✅ 16/16（會被導覽列蓋住，已全數移除）|
| 站徽帶 `aria-label` | ✅ 16/16 |
...
目前**沒有內容缺口**。`advice` 已於 2026-08-03 補齊全部 14 頁（定義見下）。
