...
| 項目 | 狀態 |
|---|---|
| 段落 id 與順序 | ✅ 18/18 為  → `map-section` → `elevation` → `spots` → `timeline` |
| 導覽鍵（五顆、規格文字） | ✅ 18/18 |
| 航點卡欄位 id | ✅ 18/18（含 `wp-pos-label`、`wp-time`、`wp-advice`；`huoyianshan` 的建議框 2026-08-03 補上，此前是唯一缺的一頁）|
| canvas id 為 `elevation-chart` | ✅ 18/18 |
| 導覽列 `fixed` | ✅ 18/18 |
| `<footer>` 在 `<main>` 外 | ✅ 18/18 |
| 地圖單欄堆疊 | ✅ 18/18 |
| 灰階用 stone | ✅ 18/18 — **class 名稱與十六進位值都查過**（2026-08-03 起）|
| 總覽有總里程 | ✅ 18/18 |
| 無 `scrollToSection()` | ✅ 18/18 |
| 站徽（綠圓 + 爬爬小隊） | ✅ 18/18 |
| 段落 `scroll-margin-top` | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | ✅ 18/18 |
| 小字對比 ≥ 4.5:1 | ✅ 18/18（含 index）0 筆失敗，2026-08-03 第八輪全面複測 |
| 無左上 `fixed` 返回鍵 | ✅ 18/18（會被導覽列蓋住，已全數移除）|
