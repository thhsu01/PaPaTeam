...
收斂已完成，15 頁的結構現在都符合本規格；仍未補齊的內容缺口見文末「尚未收斂的一件事」。
...
| 項目 | 狀態 |
|---|---|
| 段落 id 與順序 | ✅ 15/15 為  → `map-section` → `elevation` → `spots` → `timeline` |
| 導覽鍵（五顆、規格文字） | ✅ 15/15 |
| 航點卡欄位 id | ✅ 15/15（含 `wp-pos-label`、`wp-time`、`wp-advice`；`huoyianshan` 的建議框 2026-08-03 補上，此前是唯一缺的一頁）|
| canvas id 為 `elevation-chart` | ✅ 15/15 |
| 導覽列 `fixed` | ✅ 15/15 |
| `<footer>` 在 `<main>` 外 | ✅ 15/15 |
| 地圖單欄堆疊 | ✅ 15/15 |
| 灰階用 stone | ✅ 15/15 — **class 名稱與十六進位值都查過**（2026-08-03 起）|
| 總覽有總里程 | ✅ 15/15 |
| 無 `scrollToSection()` | ✅ 15/15 |
| 站徽（綠圓 + 爬爬小隊） | ✅ 15/15 |
| 段落 `scroll-margin-top` | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | ✅ 15/15 |
| 小字對比 ≥ 4.5:1 | ✅ 15/15（含 index）0 筆失敗，2026-08-03 第八輪全面複測 |
| 無左上 `fixed` 返回鍵 | ✅ 15/15（會被導覽列蓋住，已全數移除）|
| 站徽帶 `aria-label` | ✅ 15/15 |
