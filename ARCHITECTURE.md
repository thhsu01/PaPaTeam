...
收斂已完成，17 頁的結構現在都符合本規格；仍未補齊的內容缺口見文末「尚未收斂的一件事」。
...
| 項目 | 狀態 |
|---|---|
| 段落 id 與順序 | ✅ 17/17 為  → `map-section` → `elevation` → `spots` → `timeline` |
| 導覽鍵（五顆、規格文字） | ✅ 17/17 |
| 航點卡欄位 id | ✅ 17/17（含 `wp-pos-label`、`wp-time`、`wp-advice`；`huoyianshan` 的建議框 2026-08-03 補上，此前是唯一缺的一頁）|
| canvas id 為 `elevation-chart` | ✅ 17/17 |
| 導覽列 `fixed` | ✅ 17/17 |
| `<footer>` 在 `<main>` 外 | ✅ 17/17 |
| 地圖單欄堆疊 | ✅ 17/17 |
| 灰階用 stone | ✅ 17/17 — **class 名稱與十六進位值都查過**（2026-08-03 起）|
| 總覽有總里程 | ✅ 17/17 |
| 無 `scrollToSection()` | ✅ 17/17 |
| 站徽（綠圓 + 爬爬小隊） | ✅ 17/17 |
| 段落 `scroll-margin-top` | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | ✅ 17/17 |
| 小字對比 ≥ 4.5:1 | ✅ 17/17（含 index）0 筆失敗，2026-08-03 第八輪全面複測 |
| 無左上 `fixed` 返回鍵 | ✅ 17/17（會被導覽列蓋住，已全數移除）|
| 站徽帶 `aria-label` | ✅ 17/17 |
