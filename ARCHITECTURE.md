...
——理由見文末「尚未收斂的三件事」。主色本身早已收成 token，複製時照抄無妨。

收斂已完成，14 頁的結構現在都符合本規格；仍未補齊的內容缺口見文末「尚未收斂的三件事」。
...
| 地圖單欄堆疊 | ✅ 14/14 |
| 灰階用 stone | ✅ 14/14（`shiqiulinling` 原本用 slate，已收斂）|
| 無 `scrollToSection()` | ✅ 14/14 |
| 站徽（綠圓 + 爬爬小隊） | ✅ 14/14 |
| 段落 `scroll-margin-top` | ✅ 集中於 `detail.css` |
| 主色收成 `--accent` token | ✅ 14/14 |
| 小字對比 ≥ 4.5:1 | ⚠️ 第六、七輪測過的 11 頁為 0 筆失敗；`nangangshan`、`datongshan`、`shanying` 尚未複測 |

**左上返回鍵是失效的**——見下方「尚未收斂的一件事」。
...
**1. 左上返回鍵被導覽列蓋住（14 頁皆然）**

骨架裡的「返回按鈕 (fixed 左上)」是 `fixed top-4 left-4 z-50`，導覽列是
`fixed top-0 w-full`、高 57px、同樣 `z-50`。兩者重疊，而導覽列在 DOM 中較後面，
同層級下後者贏——返回鍵因此完全看不見也點不到。
2026-08-03 以 `elementFromPoint` 在 1280px 與 390px 兩種視窗實測 14 頁，
**14/14 皆被導覽列的站徽連結蓋住**。

實務上使用者仍回得了首頁（站徽本身就連到 `index.html`），所以不是功能中斷，
但那 14 份 markup 是死的，而且檢查清單裡的「返回首頁連結有效」一直是假的通過。
要收斂只有兩條路，擇一即可：拿掉返回鍵、讓站徽當唯一的回首頁入口；或把返回鍵
移出導覽列覆蓋範圍（例如挪進導覽列內部當第一顆按鈕）。**尚未決定，先如實記著。**

**2. 對比度複測未涵蓋新頁**

`nangangshan`、`datongshan`、`shanying` 是第七輪之後才建的，尚未跑過對比度掃描。

**3. 內容缺口**（不是結構問題，需要實地資料才能補）
...
- [ ] 海拔圖表繪製完整
- [ ] 上/下一個航點鍵可切換，地圖標記同步
- [ ] 回首頁的路徑有效——**用導覽列的站徽測，不要測左上的返回鍵**，後者被導覽列蓋住（見「尚未收斂的三件事」）
- [ ] 手機版本響應式正常
- [ ] 無 JavaScript 控制台錯誤
...
**最後更新**：2026-08-03（14 頁結構已全數收斂；補記返回鍵被導覽列蓋住的問題）  
