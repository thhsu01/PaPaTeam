...
返回鍵是 `fixed top-4 left-4 z-50`，導覽列是 `fixed top-0 w-full`、高 57px、同樣
`z-50`。同層級下 DOM 較後者贏，導覽列因此把返回鍵整個蓋掉。以 `elementFromPoint`
在 1280px 與 390px 兩種視窗實測 14 頁，**14/14 的返回鍵中心點打到的都是導覽列的
站徽連結**，不是返回鍵本身。

不是功能中斷——站徽本身就連到 `index.html`，使用者照樣回得了首頁。但那 14 份
markup 是死的，且檢查清單裡「返回首頁連結有效」一直是假的通過。
處置方式未定，見 `ARCHITECTURE.md`「尚未收斂的三件事」。

### 對比度複測未涵蓋三個新頁

`nangangshan`、`datongshan`、`shanying` 建於第七輪之後，尚未跑過對比度掃描。
