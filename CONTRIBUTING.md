...
`dist` 是實走才知道的東西，**不得為了填滿欄位而編造**——沒量到就留空。
座標對不上時同樣如實留著疑點，等走過的人確認，不要挑一個看起來合理的數字改掉。

`advice` 則是每個航點都要有：候選頁依公開資訊寫「這段路要注意什麼」，走過之後
換成隊友的實際經驗。空的建議框刻意不隱藏，漏寫時就該在畫面上看得見——
`spec_sweep.py` 也會把缺 `advice` 的航點逐一列出來。
...
- **改了 `assets/` 的共用檔就跑 `python3 tools/bump_assets.py`**，讓 23 頁的 `?v=` 換成
  ...
  `spec_sweep` 擋下來
- **新增或刪除檔案只改 `manifest.json`**（條目的 `short` 是檔案樹上那一句），然後跑
  `python3 tools/manifest_tree.py --write` 讓 README 的檔案樹跟上。README 的樹是產生的，
  不要手改；漏條目或樹過期都會被 `spec_sweep` 擋下來
...
計畫頁只有 `date`、沒有 `km`——「紀錄頁」的判準是 `trip.km`（`detail.js` 的 `isRecord()`
與 `spec_sweep` 同一個），所以定了日期的計畫頁仍是行前頁：第五鍵「預估進度」、天氣卡查
行程日預報、不放 `notice` 掃載點。候選頁連 `date` 都沒有，不要為了湊規格編一個。
